...
if ("$event" eq "") {
    ...
    if ("$pos" eq "") {
	print "<H2>Pick your field orientation and Robot position</H2>\n";
	...
	print "</tr></table>\n";
		...
		for (my $m = 1; $m < $count; $m++) {
			...
			if (defined $mhash{$key}) {
			    if ($orient eq "right" && ($pos eq "R1" || $pos eq "R2" || $pos eq "R3")) {
				print "<td><p style=\"font-size:20px; font-weight:bold;\"><a href=\"red_right.cgi?game=${key}&team=$mhash{$key}\">$pos : $mhash{$key}</a></p></td>\n";
			    } elsif ($orient eq "left" && ($pos eq "R1" || $pos eq "R2" || $pos eq "R3")) {
				print "<td><p style=\"font-size:20px; font-weight:bold;\"><a href=\"red_left.cgi?game=${key}&team=$mhash{$key}\">$pos : $mhash{$key}</a></p></td>\n";
			    } elsif ($orient eq "left" && ($pos eq "B1" || $pos eq "B2" || $pos eq "B3")) {
				print "<td><p style=\"font-size:20px; font-weight:bold;\"><a href=\"blue_left.cgi?game=${key}&team=$mhash{$key}\">$pos : $mhash{$key}</a></p></td>\n";
			    } else {
				# blue right
				print "<td><p style=\"font-size:20px; font-weight:bold;\"><a href=\"blue_right.cgi?game=${key}&team=$mhash{$key}\">$pos : $mhash{$key}</a></p></td>\n";
			    }
			} else {
